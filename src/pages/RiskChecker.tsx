import { AlertTriangle, ChevronLeft, ChevronRight, Grid, MapPin } from 'lucide-react';
...
const questions: Question[] = [
  {
    id: 'animalType',
    question: { en: 'What type of animals do you primarily raise?', hi: 'Aap mukhya roop se kaun se jaanwar paaltē hain?' },
    options: [
      { value: 1, label: { en: 'Poultry', hi: 'Murgi' } },
      { value: 2, label: { en: 'Pigs', hi: 'Suar' } },
      { value: 3, label: { en: 'Mixed', hi: 'Mixed' } },
      { value: 4, label: { en: 'Other livestock', hi: 'Anya jaanwar' } }
    ]
  },
  {
    id: 'visitors',
    question: { en: 'How many visitors come to your farm per week?', hi: 'Hafte mein kitne visitors aapke farm par aate hain?' },
    options: [
  ...
  {
    id: 'entryControls',
    question: { en: 'Do you have entry controls for visitors?', hi: 'Kya aapke paas visitors ke liye entry controls hain?' },
    options: [
  ...
  {
    id: 'feedSource',
    question: { en: 'Where do you primarily source your animal feed?', hi: 'Aap apna animal feed mukhya roop se kahan se laate hain?' },
    options: [
  ...
  {
    id: 'sickAnimals',
    question: { en: 'Have you had sick animals in the past 3 months?', hi: 'Kya pichle 3 mahine mein aapke paas bimar jaanwar the?' },
    options: [
  ...
  {
    id: 'proximity',
    question: { en: 'How far is your farm from the nearest livestock market?', hi: 'Aapka farm sabse nazdeeki livestock market se kitni door hai?' },
    options: [
...
const RiskChecker = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      const { answers: savedAnswers, step, selectedState: state, system: sys } = JSON.parse(saved);
  ...
  const calculateRisk = () => {
    ...
    const totalScore = baseScore + epiModifier;
  ...
  if (showResult) return <RiskResult answers={answers} onRestart={resetAssessment} />;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">{t('risk.title')}</h1>
          <p className="text-gray-600">{t('risk.subtitle')}</p>
        </div>

        {/* Progress Bar & Selects */}
        <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8 mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div>
              <label htmlFor="stateSelect" className="flex items-center text-sm font-medium text-gray-600 mb-2">
                <MapPin className="h-4 w-4 mr-2" /> {t('risk.location')}
              </label>
              <select
                id="stateSelect"
                aria-label={t('risk.location')}
            ...
            <div>
              ...
              <div className="flex gap-2">
                {(['poultry', 'pigs', 'mixed'] as ProductionSystem[]).map(sys => (
                  <button
                    key={sys}
                    type="button"
                  ...
                  >
                    {sys === 'poultry' ? 'Poultry' : sys === 'pigs' ? 'Pigs' : 'Mixed'}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div className="bg-gradient-to-r from-teal-500 to-blue-500 h-2 rounded-full transition-all duration-300" style={{ width: `${progress}%` }} />
          </div>
        </div>

        {/* Question Card */}
        <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
          <h2 className="text-xl font-semibold text-gray-800 mb-6">{currentQuestion.question[language]}</h2>
          <div className="space-y-3">
            {currentQuestion.options.map((option, index) => (
              <label key={index} className={`flex items-center p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 hover:border-teal-200 hover:bg-teal-50 ${answers[currentQuestion.id] === option.value ? 'border-teal-500 bg-teal-50' : 'border-gray-200'}`}>
                <input
                  type="radio"
                  ...
                  className="sr-only"
                />
                <div className={`w-4 h-4 rounded-full border-2 mr-4 flex items-center justify-center ${answers[currentQuestion.id] === option.value ? 'border-teal-500 bg-teal-500' : 'border-gray-300'}`}>
                  {answers[currentQuestion.id] === option.value && <div className="w-2 h-2 rounded-full bg-white" />}
                </div>
                <span className="text-gray-700 font-medium">{option.label[language]}</span>
              </label>
            ))}
          </div>
          ...
          <div className="flex justify-between mt-8">
            <button onClick={goBack} disabled={currentStep === 0} className={`flex items-center px-6 py-3 rounded-full font-medium transition-all ${currentStep === 0 ? 'text-gray-400 cursor-not-allowed' : 'text-gray-600 hover:text-teal-600 hover:bg-teal-50'}`}>
              <ChevronLeft className="mr-2 h-4 w-4" />
              {t('btn.back')}
            </button>
            <button onClick={goNext} disabled={!answers[currentQuestion.id]} className={`flex items-center px-6 py-3 rounded-full font-medium transition-all ${answers[currentQuestion.id] ? 'bg-gradient-to-r from-teal-600 to-blue-600 text-white hover:shadow-lg hover:scale-105' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
              ...
              {assessments.slice(0, 3).map((assessment) => (
                <div key={assessment.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center">
                    <div className={`w-3 h-3 rounded-full mr-3 ${assessment.riskLevel === 'low' ? 'bg-green-500' : assessment.riskLevel === 'medium' ? 'bg-yellow-500' : 'bg-red-500'}`} />
                    <span className="text-sm font-medium text-gray-700">{assessment.farmName}</span>
                  </div>
