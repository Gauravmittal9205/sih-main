import { Bell, CheckCircle, FileText, HelpCircle, LayoutDashboard, LogOut, Menu, Shield, User as UserIcon, BarChart3, Brain } from 'lucide-react';
...
const DashboardStats: React.FC<{
  ...
  return (
    <div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className={`p-5 rounded-lg border ${scoreColor}`}>
          ...
          <div className="mt-3">
            ...
            {/* Assessment and ML Analysis Buttons */}
            <div className="mt-3 space-y-2">
              ...
              {isAssessmentComplete && (
                <button
                  onClick={() => onGetMLInsights(biosecurityData)}
                  disabled={mlLoading}
                  className="w-full bg-purple-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-purple-700 transition-colors disabled:opacity-50"
                >
                  {mlLoading ? 'Analyzing...' : 'Get AI Insights'}
                </button>
              )}
            </div>
      ...
      {mlResults && (
        <div className="mt-6 bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          <div className="flex items-center space-x-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-gray-800">AI-Powered Biosecurity Insights</h3>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-gradient-to-br from-blue-50 to-purple-50 rounded-lg p-4 border border-blue-200">
              <div className="text-center">
                <h4 className="text-sm font-medium text-blue-800 mb-2">ML Biometric Score</h4>
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 text-white mb-2">
                  <span className="text-xl font-bold">{mlResults.biometric_score || 'N/A'}</span>
                </div>
                <p className="text-xs text-blue-600">out of 100</p>
                <div className={`mt-2 inline-block px-2 py-1 rounded-full text-xs font-medium ${
                  mlResults.risk_level === 'High' ? 'bg-red-100 text-red-800' :
                  mlResults.risk_level === 'Medium' ? 'bg-yellow-100 text-yellow-800' :
            ...
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-lg p-4 border border-amber-200">
              <h4 className="text-sm font-medium text-amber-800 mb-3">Risk Analysis</h4>
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-xs">
                  ...
                  <span className={`font-semibold px-2 py-1 rounded-full text-xs ${
                    mlResults.risk_level === 'High' ? 'bg-red-100 text-red-700' :
                    mlResults.risk_level === 'Medium' ? 'bg-yellow-100 text-yellow-700' :
                    mlResults.risk_level === 'Low' ? 'bg-green-100 text-green-700' :
                ...
                {mlResults.detailed_analysis?.priority_areas && mlResults.detailed_analysis.priority_areas.length > 0 && (
                  <div className="mt-2">
                    <div className="text-xs text-amber-700 mb-1">Priority Areas:</div>
            ...
            <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg p-4 border border-green-200">
              <h4 className="text-sm font-medium text-green-800 mb-3">Category Performance</h4>
              <div className="space-y-2">
                    ...
                    <div key={category} className="flex justify-between items-center">
                      ...
                      <div className="flex items-center space-x-2">
                        <div className="w-16 h-2 bg-green-100 rounded-full overflow-hidden">
                          <div 
                            className={`h-full ${
                              scoreValue >= 15 ? 'bg-green-500' : 
                              scoreValue >= 10 ? 'bg-yellow-500' : 'bg-red-500'
                            }`} 
                            style={{ width: `${(scoreValue / 20) * 100}%` }}
                          />
                        </div>
                        <span className="text-xs font-semibold text-green-700">{scoreValue}/20</span>
                      </div>
                    </div>
                  );
                })}
          ...
          {/* ML Recommendations */}
          <div className="mt-6">
            <h4 className="text-sm font-medium text-gray-800 mb-3">AI Recommendations</h4>
                ...
                <div className="text-gray-500 text-sm">
                  {mlResults.error ? (
                    <div className="flex items-center justify-center space-x-2">
                      ...
                      <span>ML Model Error: {mlResults.error}</span>
                    </div>
                  ) : (
                    <div className="flex items-center justify-center space-x-2">
      ...
      {/* Enhanced ML Analytics with Graphs */}
      {mlResults && (
        <div className="mt-6 bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          <div className="flex items-center space-x-2 mb-4">
            <BarChart3 className="h-5 w-5 text-purple-600" />
            <h3 className="text-lg font-semibold text-gray-800">ML Model Analytics & Graphs</h3>
          </div>
          
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* ML Category Performance Chart */}
            <div className="bg-gradient-to-br from-purple-50 to-blue-50 rounded-lg p-4 border border-purple-200">
              <h4 className="text-sm font-medium text-purple-800 mb-3">ML Category Scores</h4>
              <div className="w-full h-48">
                <svg viewBox="0 0 300 180" className="w-full h-full">
                  {mlResults.category_scores && Object.entries(mlResults.category_scores).map(([category, score], index) => {
                    const scoreValue = score as number;
                    const x = (index / (Object.keys(mlResults.category_scores).length - 1)) * 280 + 10;
                    const height = (scoreValue / 20) * 120;
                    const y = 160 - height;
                    const barWidth = 40;
                    const color = scoreValue >= 15 ? '#10b981' : scoreValue >= 10 ? '#f59e0b' : '#ef4444';
                  ...
                  <line
                    x1="0"
                    y1={mlResults.risk_level === 'High' ? 80 : mlResults.risk_level === 'Medium' ? 60 : 40}
                    x2="300"
                    y2={mlResults.risk_level === 'High' ? 80 : mlResults.risk_level === 'Medium' ? 60 : 40}
                    stroke="#ef4444"
                    strokeWidth="2"
                    strokeDasharray="5,5"
                  />
                  <text
                    x="5"
                    y={mlResults.risk_level === 'High' ? 75 : mlResults.risk_level === 'Medium' ? 55 : 35}
                    fontSize="8"
                    fill="#ef4444"
                    fontWeight="bold"
                  >
            ...
            {/* ML Confidence & Risk Distribution */}
            <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-lg p-4 border border-emerald-200">
              <h4 className="text-sm font-medium text-emerald-800 mb-3">AI Confidence & Risk</h4>
              <div className="w-full h-48">
                <svg viewBox="0 0 300 180" className="w-full h-full">
                  {/* Confidence circle */}
                  <circle
                  ...
                  <circle
                    ...
                    stroke="#10b981"
                    strokeWidth="8"
                    strokeDasharray={`${(mlResults.confidence || 0) * 314} 314`}
                  ...
                  <rect
                    ...
                    width="80"
                    height="20"
                    fill={mlResults.risk_level === 'High' ? '#ef4444' : mlResults.risk_level === 'Medium' ? '#f59e0b' : '#10b981'}
                    rx="4"
                  />
                  <text
                    x="230"
          ...
          {/* Categorized ML Outputs */}
          <div className="mt-6 pt-6 border-t border-gray-200">
            <h4 className="text-sm font-medium text-gray-800 mb-4">Categorized ML Analysis</h4>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* Risk Assessment */}
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <h5 className="text-sm font-medium text-red-800 mb-2">Risk Assessment</h5>
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between">
                    <span>Level:</span>
                    <span className="font-semibold text-red-700">{mlResults.risk_level || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Score:</span>
                    <span className="font-semibold text-red-700">{mlResults.biometric_score || 'N/A'}/100</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Confidence:</span>
                    <span className="font-semibold text-red-700">{mlResults.confidence ? Math.round(mlResults.confidence * 100) : 'N/A'}%</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <h5 className="text-sm font-medium text-blue-800 mb-2">Performance Metrics</h5>
                <div className="space-y-2 text-xs">
                  {mlResults.category_scores && Object.entries(mlResults.category_scores).slice(0, 3).map(([category, score]) => {
                    const scoreValue = score as number;
                    return (
                      <div key={category} className="flex justify-between">
                        <span>{category}:</span>
                        <span className={`font-semibold ${
                          scoreValue >= 15 ? 'text-green-700' : 
                          scoreValue >= 10 ? 'text-yellow-700' : 'text-red-700'
                        }`}>
                          {scoreValue}/20
                        </span>
                      </div>
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <h5 className="text-sm font-medium text-green-800 mb-2">AI Predictions</h5>
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between">
                    <span>Prediction:</span>
                    <span className="font-semibold text-green-700">{mlResults.prediction || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Priority Areas:</span>
                    <span className="font-semibold text-green-700">
                      {mlResults.detailed_analysis?.priority_areas?.length || 0}
                    </span>
                  </div>
                  <div className="flex justify-between">
